...
function loadUserData(user) {
    ...
    if (memberNumberElement) {
        memberNumberElement.textContent = user.numero_socio || 'U2025XXXXX';
    }
    ...
    if (currentPlanNameElement) {
        currentPlanNameElement.textContent = `Membresía ${planData.name}`;
    }
    
    // Show "Gratis" for free plan, or price for paid plans
    const currentPlanPriceElement = document.getElementById('currentPlanPrice');
    if (currentPlanPriceElement) {
        if (currentPlan === 'free') {
            currentPlanPriceElement.textContent = 'Gratis';
        } else {
            currentPlanPriceElement.textContent = `S/ ${planData.price.toFixed(2)} / año`;
        }
    }
